...
class OC():
    ...
    def get_optimized_external_potential(self, grid_info, average_alpha_beta=False):
        """
        $
        ...
        vxc_LDA_DFT = self.eng.grid.vxc(func_id=1, Da=Da_LDA, Db=Db_LDA)
        vxc_LDA_DFT += self.eng.grid.vxc(func_id=8, Da=Da_LDA, Db=Db_LDA)
        ...
        e_bar_DFT = self.eng.grid.average_local_orbital_energy(Da_LDA, Ca_LDA[:,:Nalpha], epsilon_a_LDA[:Nalpha], Vpot=Vpot)
        e_bar = self.eng.grid.average_local_orbital_energy(Da_LDA, Ca_LDA[:, :Nalpha], epsilon_a_LDA[:Nalpha], grid_info=grid_info, Vpot=Vpot)

        tauLmP_rho_DFT = self.eng.grid.get_l_kinetic_energy_density_directly(Da_LDA, Ca_LDA[:,:Nalpha], Vpot=Vpot)
        tauLmP_rho = self.eng.grid.get_l_kinetic_energy_density_directly(Da_LDA, Ca_LDA[:,:Nalpha], grid_info=grid_info, Vpot=Vpot)

        tauP_rho_DFT = self.eng.grid.pauli_kinetic_energy_density(Da_LDA, Ca_LDA[:,:Nalpha], Vpot=Vpot)
        tauP_rho = self.eng.grid.pauli_kinetic_energy_density(Da_LDA, Ca_LDA[:,:Nalpha], Vpot=Vpot, grid_info=grid_info)
        ...
        J = self.eng.compute_hartree(Ca_LDA[:,:Nalpha],  Cb_LDA[:,:Nbeta])[0]
        vext_opt_no_H_DFT_Fock = self.eng.grid.dft_grid_to_fock(vext_opt_no_H_DFT)
        ...
        vH = self.eng.grid.esp(Da=Da_LDA, Db=Db_LDA, grid=grid_info)[1]
        ...
        if self.ref != 1:
            ...
            tauP_rho_DFT_beta = self.eng.grid.pauli_kinetic_energy_density(Db_LDA, Cb_LDA[:,:Nbeta])
            tauP_rho_beta = self.eng.grid.pauli_kinetic_energy_density(Db_LDA, Cb_LDA[:,:Nbeta], grid_info=grid_info)
            ...
            vext_opt_no_H_DFT_Fock_beta = self.dft_grid_to_fock(vext_opt_no_H_DFT_beta)
    ...
    def oucarter(self, maxiter, vxc_grid, D_tol=1e-7,
        ...
        if self.ref == 1:
            vext_opt_Fock, vext_opt = self.get_optimized_external_potential(grid_info)
        else:
            (vext_opt_Fock, vext_opt_Fock_beta), (vext_opt, vext_opt_beta) = self.get_optimized_external_potential(grid_info)
        ...
        if init is None:
            ...
            wfn_temp = psi4.energy(init+"/" + self.eng.basis_str, 
                                   molecule=self.eng.mol, 
                                   return_wfn=True)[1]
        ...
        Da_old = 0.0
        eig_old = 0.0
        tauLmP_rho = self.eng.grid.get_l_kinetic_energy_density_directly(self.Dt[0], self.ct[0][:, :Nalpha])
        if self.ref != 1:
            tauLmP_rho_beta = self.eng.grid.get_l_kinetic_energy_density_directly(self.Dt[1], self.ct[1][:, :Nbeta])

        for OC_step in range(1, maxiter+1):
            tauP_rho = self.eng.grid.pauli_kinetic_energy_density(self.Da, self.Coca)
            # _average_local_orbital_energy() taken from mrks.py.
            e_bar = self.eng.grid.average_local_orbital_energy(self.Da, self.Coca, self.eigvecs_a[:Nalpha])
            shift = self.eigvecs_a[Nalpha - 1] - self.wfn.epsilon_a().np[Nalpha - 1]
            # vxc + vext_opt + vH0
            vxc_extH = e_bar - tauLmP_rho - tauP_rho - shift

            if self.ref != 1:
                tauP_rho_beta = self.eng.grid.pauli_kinetic_energy_density(self.Db, self.Cocb)
                e_bar_beta = self.eng.grid.average_local_orbital_energy(self.Db, self.Cocb, self.eigvecs_b[:Nbeta])
            ...
            Vxc_extH_Fock = self.eng.grid.dft_grid_to_fock(vxc_extH)
            Vxc_Fock = Vxc_extH_Fock - vext_opt_Fock - vH0_Fock

            if self.ref != 1:
                Vxc_extH_Fock_beta = self.eng.grid.dft_grid_to_fock(vxc_extH_beta)
        ...
        vH0 = self.eng.grid.esp(grid=grid_info)[1]
        tauLmP_rho = self.eng.grid.get_l_kinetic_energy_density_directly(self.Dt[0], self.ct[0][:, :Nalpha], grid_info=grid_info)
        tauP_rho = self.eng.grid.pauli_kinetic_energy_density(self.Da, self.Coca, grid_info=grid_info)
        shift = self.eigvecs_a[Nalpha - 1] - self.wfn.epsilon_a().np[Nalpha - 1]
        e_bar = self.eng.grid.average_local_orbital_energy(self.Da, self.Coca, self.eigvecs_a[:Nalpha], grid_info=grid_info)
        if self.ref != 1:
            tauLmP_rho_beta = self.eng.grid.get_l_kinetic_energy_density_directly(self.Dt[1], self.ct[1][:, :Nbeta],
                                                                     grid_info=grid_info)
            tauP_rho_beta = self.eng.grid.pauli_kinetic_energy_density(self.Db, self.Cocb, grid_info=grid_info)
            shift_beta = self.eigvecs_b[Nbeta - 1] - self.wfn.epsilon_b().np[Nbeta - 1]
            e_bar_beta = self.eng.grid.average_local_orbital_energy(self.Db, self.Cocb, self.eigvecs_b[:Nbeta], grid_info=grid_info)

        if self.ref == 1:
            grid_vxc = e_bar - tauLmP_rho - tauP_rho - vext_opt - vH0 - shift
            return grid_vxc, e_bar, tauLmP_rho, tauP_rho, vext_opt, vH0, shift
        else:
            grid_vxc = np.array((e_bar - tauLmP_rho - tauP_rho - vext_opt - vH0 - shift,
                                      e_bar_beta - tauLmP_rho_beta - tauP_rho_beta - vext_opt_beta - vH0 - shift_beta
                                      ))
            return grid_vxc, (e_bar, e_bar_beta), (tauLmP_rho, tauLmP_rho_beta), \
